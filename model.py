...
from model_cfg import get_config
from GhostBneck import GhostModule
...
class MobileViT(nn.Module):
    ...
    """
    def __init__(self, model_cfg: Dict, num_classes: int = 1000, image_channels:int = 1):
        super().__init__()
...
class GhostViT(nn.Module):
    ...
    def _make_layer(self, input_channel, cfg: Dict) -> Tuple[nn.Sequential, int]:
        ...
        if block_type.lower() == "mobilevit":
            ...
            return self._make_ghostnet_layer(input_channel=input_channel, cfg=cfg)

    @staticmethod
    def _make_ghostnet_layer(input_channel: int, cfg: Dict) -> Tuple[nn.Sequential, int]:
        output_channels = cfg.get("out_channels")
        num_blocks = cfg.get("num_blocks", 2)
        expand_ratio = 2
        block = []
...
def mobile_vit_xx_small(num_classes: int = 1000, in_channels: int=1):
    ...
    m = MobileViT(config, num_classes=num_classes, image_channels=in_channels)
    return m

def ghost_vit(num_classes: int = 1000, in_channels: int=1):
    config = get_config("xx_small")
    m = GhostViT(config, num_classes=num_classes, image_channels=in_channels)
    return m

def mobile_vit_x_small(num_classes: int = 1000, in_channels: int=1):
    # pretrain weight link
...
def mobile_vit_small(num_classes: int = 1000, in_channels: int=1):
    # pretrain weight link
...
def mlp(num_classes: int = 1000, in_channels: int=1):
    m = MLP(num_classes, in_channels)
    return m